...
flowchart TB
    ...
    RT[Router ACP<br/>JSON 分类]
    DISP[router_dispatch.go<br/>机械分发]
    PS[scripts/*.ps1 固定脚本]
    SP[Specialist ACP<br/>opencode]
    DEL[WECLAW_DELEGATE 解锁委派]
    NORM[reply_normalize.go 最终规范化]
...
| 层 | 职责 | 禁止 |
|----|------|------|
| **WeClaw (Go)** | iLink 收发、会话记忆、进度转发、解锁委派、回复规范化 | 按关键词替大脑选脚本 |
| **OpenCode + AGENTS** | 意图识别、选 skill、调一个固定脚本 | 即兴写 PowerShell、直接跑 unlock |
| **scripts/*.ps1** | 执行单一 PC 动作，输出机器可读前缀 | 无限循环、Agent 在聊天里改脚本 |
