...
| **Cursor Worker** | 复杂任务 → `cursor-delegate-task.ps1` → Cursor 对话框 → OCR 监测 → 微信摘要 | 用 OpenCode/Windows-Use 做编码任务 |

> **§81 Cursor Worker：** 非机械、非日常闲聊 → Go 强制 `cursor_worker`；额度用尽 → `D:\AI助手.bat` 换号，等 2 分钟后重发；统一对话 **工作 › WeChat account transfer**。
