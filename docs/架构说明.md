...
| **inbound 附件层** | 微信文件 → `~/.weclaw/inbound/{user}/` → Planner 带路径 | 让 Specialist 猜文件路径 |
| **脚本注册表** | `data/registry.sqlite` + `config/script-manifest.json` 沉淀脚本/skill/操作日志 | 临场发明流程、Cursor 桌面自动化 |

> **§82 架构回退：** 停用 Cursor Worker（§80–81）。**双通道：** ① 机械/可脚本化 → Planner 直跑 `scripts/*.ps1`；② 日常闲聊与未覆盖任务 → **OpenCode 单通道**（skills + 必要时**新建脚本**再注册）。详见 `python scripts/list-registry.py --lessons`。
