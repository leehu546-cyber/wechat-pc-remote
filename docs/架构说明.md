...
**当前主路径：** 手机微信 → `weclaw.exe` → `opencode acp` → `deepseek/deepseek-v4-flash`（DeepSeek 付费 API）
...
flowchart TB
    ...
    MEM[本地 chat-log 记忆]
    ACP[OpenCode ACP + DeepSeek API]
    SK[AGENTS.md + skills]
    PS[scripts/*.ps1 固定脚本]
    DEL[WECLAW_DELEGATE 解锁执行]
...
**设计原则：薄桥 + 单大脑（全语义经 DeepSeek API）**

| 层 | 职责 | 禁止 |
|----|------|------|
| **WeClaw (Go)** | 去重、iLink 收发、记忆、解锁**执行**、回复规范化 | Go JSON Router、关键词选脚本、直跑 screenshot 等 |
| **OpenCode + AGENTS + skills** | **唯一**语义入口：理解、选 skill、调 bash | 第二套 LLM Router、Codex 替代主路径 |
| **scripts/*.ps1** | 机械执行 + `WECHAT_USER_REPLY` | Agent 在聊天里改脚本 |

> **已删除（勿恢复）：** Plan A `router.go` / `router_dispatch.go` — Go 层 JSON 分类 + 绕过 OpenCode 直跑脚本。
...
WeClaw **不做**关键词路由；下表由 **Agent 按语义**匹配。

| 用户意图（示例） | Skill | 唯一脚本 | 固定回复 |
|------------------|-------|----------|----------|
| 截图 / 截屏 | `wechat-screenshot` | `screenshot.ps1` | **无文字**（`WECHAT_MEDIA_SENT`，脚本已发图） |
| 看屏幕 / 检索 / OCR / 屏幕上有什么 | `wechat-screen-ocr` | `screen-ocr.ps1` | `屏幕上主要是：{≤40字}` |
| 亮屏 / 开屏 | `wechat-screen-on` | `wake-screen.ps1` | `屏幕已点亮。` |
| 关屏 / 熄屏 | `wechat-screen-off` | `turn-off-screen.ps1` | `显示器已关闭。` |
| 解锁 / 进桌面 / 锁屏输密码 | `wechat-screen-unlock` | **委派**（无 bash） | `已解锁，请看屏幕。` |
| 股票 / 持仓 / 510300 | `wechat-stock-info` | `stock-info.ps1` | 原样 `WECHAT_STOCK_CARD` |
| 放歌 / 听歌 | `bilibili-music` | 搜索 + Edge | `已在浏览器打开：{歌名}` |
| 打开文件 | （固定 opener） | `open-file-fast.ps1` | `已打开：{文件名}` |
| 在应用里输入文字 | `wechat-desktop-interaction` | `desktop-interact.ps1` | `已完成：…` |
