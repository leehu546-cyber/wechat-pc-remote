# Test unlock methods (safe: does not print password). Run unlocked desktop to verify mechanics.
param([switch]$Quiet)
...
function Test-SchtasksSendKeys {
    ...
    if ($runOut -match 'SUCCESS|成功') {
        return @{ ok = $true; detail = "schtasks run ok ($RunAs $level SendKeys -File)" }
    }
...
Write-TestLog "=== unlock method test ==="
...
$results += [PSCustomObject]@{ Method = "B_schtasks_USER_HIGHEST"; Ok = $r2.ok; Detail = $r2.detail }
Write-TestLog "B schtasks USER HIGHEST: $($r2.ok) | $($r2.detail)"

$r3 = Test-SchtasksSendKeys -Name "SystemHighest" -RunAs "SYSTEM"
$results += [PSCustomObject]@{ Method = "C_schtasks_SYSTEM_HIGHEST"; Ok = $r3.ok; Detail = $r3.detail }
Write-TestLog "C schtasks SYSTEM HIGHEST: $($r3.ok) | $($r3.detail)"
...
# B = first log success (03:44); C/D fail on lock screen in practice
$winner = $results | Where-Object { $_.Ok -and $_.Method -eq "B_schtasks_USER_HIGHEST" } | Select-Object -First 1
if (-not $winner) {
    $winner = $results | Where-Object { $_.Ok -and $_.Method -eq "C_schtasks_SYSTEM_HIGHEST" } | Select-Object -First 1
}
if (-not $winner) {
